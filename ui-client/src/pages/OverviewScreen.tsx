		...
		<Tabs defaultValue="overview">
			<TabsList>
				...
				<TabsTrigger value="repos">
					<AlbumIcon className="w-5 h-5" /> <span className="ml-2">Projects</span>
				</TabsTrigger>
			...
			<div className="col-span-7">
				...
				{projects.length === 0 ? (
					<div className="flex flex-col items-center justify-center py-10">
						...
						<Link to="/app/create-new-project" className="mt-4 text-blue-600 hover:underline">
...
const ProjectsTab = ({ projects }: { projects: ResearchInfo[] }) => {
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-10 gap-5 mt-5">
			...
			<div className="col-span-7">
				<div className="">
					<input className="px-2 py-1.5 outline-none border-2 border-gray-500 rounded-xl w-full focus:border-blue-700" placeholder="Find project" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
				</div>
			...
			<div className="flex items-start justify-between">
				<div className="flex-1">
					<div className="flex items-center gap-2 mb-2">
						...
						<Link to={`/app/view-project/${project.id}`} className="font-medium text-blue-600 hover:underline">
							{project.title}
						</Link>
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-2">
					<Link to={`/app/view-project/${project.id}`}>
						<h3 className="text-blue-600 font-semibold hover:underline">{project.title}</h3>
					</Link>
					<Badge variant={"outline"}>{project.isActive ? "Public" : "Private"}</Badge>
				</div>
	...
	return (
		<>
			<Avatar className="w-80 h-80 border">
				<AvatarImage src={`https://api.dicebear.com/9.x/adventurer/svg?seed=${address ? "shadcn" : "Vivian"}`} alt="shadcn" />
			...
			<div className="flex items-center gap-2 mt-2">
				{isAuthenticated && (
					<>
						...
						<CopyToClipboardBtn text={address!} customToastText="Wallet Address Copied to clipboard" />
					</>
				)}
			</div>
			<div className="mt-5"></div>
